        ...
        <div class="container">
            <?php if ( ! empty( $city_terms ) ) : ?>
                ...
                get_template_part( 'template-parts/components/section-title', null, [
                    'heading' => __( 'Browse Wedding Venues by City', 'sdweddingdirectory' ),
                    'desc'    => __( 'Start with the areas couples search most often across San Diego County.', 'sdweddingdirectory' ),
                ] );
                ?>
                    ...
                    <div class="venues-carousel__track">
                        <?php foreach ( $city_terms as $city_term ) : ?>
                            <a class="venues-carousel__slide" href="<?php echo esc_url( sdwdv2_get_venue_directory_url( [ 'location' => $city_term->slug ] ) ); ?>">
                                ...
                                <span class="venues-carousel__count"><?php echo esc_html( sprintf( _n( '%s venue', '%s venues', $city_term->count, 'sdweddingdirectory' ), number_format_i18n( $city_term->count ) ) ); ?></span>
                            </a>
                        <?php endforeach; ?>
                    </div>
                </div>
            ...
            <?php if ( ! empty( $type_terms ) ) : ?>
                <?php
                get_template_part( 'template-parts/components/section-title', null, [
                    'heading' => __( 'Explore Venue Types', 'sdweddingdirectory' ),
                    'desc'    => __( 'Use the venue directory as a starting point, then narrow by type, setting, and guest count.', 'sdweddingdirectory' ),
                ] );
                ?>
        ...
        <form class="venues-search-form" method="get" action="<?php echo esc_url( $current_url ); ?>">
            <?php if ( $fixed_category_id && $selected_category && ! is_wp_error( $selected_category ) ) : ?>
                <div class="venues-search-form__chip">
                    <span class="venues-search-form__chip-label"><?php esc_html_e( 'Venue type', 'sdweddingdirectory' ); ?></span>
                    <strong><?php echo esc_html( $selected_category->name ); ?></strong>
                </div>
            <?php else : ?>
                <label class="venues-search-form__field">
                    <span class="screen-reader-text"><?php esc_html_e( 'Venue type', 'sdweddingdirectory' ); ?></span>
                    <select name="cat_id">
                        <option value=""><?php esc_html_e( 'All venue types', 'sdweddingdirectory' ); ?></option>
            ...
            <?php if ( $fixed_location_slug && $selected_location && ! is_wp_error( $selected_location ) ) : ?>
                <div class="venues-search-form__chip">
                    <span class="venues-search-form__chip-label"><?php esc_html_e( 'Location', 'sdweddingdirectory' ); ?></span>
                    <strong><?php echo esc_html( $selected_location->name ); ?></strong>
                </div>
            <?php else : ?>
                <label class="venues-search-form__field">
                    <span class="screen-reader-text"><?php esc_html_e( 'Location', 'sdweddingdirectory' ); ?></span>
                    <select name="location">
                        <option value=""><?php esc_html_e( 'All locations', 'sdweddingdirectory' ); ?></option>
            ...
            <label class="venues-search-form__field">
                <span class="screen-reader-text"><?php esc_html_e( 'Sort venues', 'sdweddingdirectory' ); ?></span>
                <select name="sort">
            ...
            <button class="btn btn--cta venues-search-form__submit" type="submit">
                <?php esc_html_e( 'Search Venues', 'sdweddingdirectory' ); ?>
            </button>
        </form>
        ...
        <div class="archive-filtered venues-browser__layout">
            <aside class="archive-filtered__sidebar">
                <form class="venues-filter-form" method="get" action="<?php echo esc_url( $current_url ); ?>">
                    ...
                    <?php if ( ! empty( $options['price_ranges'] ) ) : ?>
                        <div class="venues-filter-form__group">
                            <label class="venues-filter-form__label" for="venues-price-filter">
                                <?php esc_html_e( 'Price range', 'sdweddingdirectory' ); ?>
                            </label>
                            <select id="venues-price-filter" name="price-filter">
                                <option value=""><?php esc_html_e( 'Any price', 'sdweddingdirectory' ); ?></option>
                    ...
                    <?php if ( ! empty( $options['capacity_ranges'] ) ) : ?>
                        <div class="venues-filter-form__group">
                            <label class="venues-filter-form__label" for="venues-capacity-filter">
                                <?php esc_html_e( 'Guest count', 'sdweddingdirectory' ); ?>
                            </label>
                            <select id="venues-capacity-filter" name="capacity">
                                <option value=""><?php esc_html_e( 'Any guest count', 'sdweddingdirectory' ); ?></option>
                    ...
                    <?php foreach ( [
                        ...
                        <fieldset class="venues-filter-form__group">
                            <legend class="venues-filter-form__label">
                                ...
                                echo esc_html(
                                    [
                                        'venue_setting'   => __( 'Setting', 'sdweddingdirectory' ),
                                        'venue_amenities' => __( 'Amenities', 'sdweddingdirectory' ),
                                        'service'         => __( 'Services', 'sdweddingdirectory' ),
                                        'style'           => __( 'Style', 'sdweddingdirectory' ),
                                    ][ $field_name ]
                                );
                                ?>
                            </legend>
                    ...
                    <button class="btn btn--primary venues-filter-form__submit" type="submit">
                        <?php esc_html_e( 'Apply Filters', 'sdweddingdirectory' ); ?>
                    </button>

                    <?php if ( $has_filters ) : ?>
                        <a class="btn btn--outline venues-filter-form__reset" href="<?php echo esc_url( $current_url ); ?>">
                            <?php esc_html_e( 'Clear Filters', 'sdweddingdirectory' ); ?>
                        </a>
                    <?php endif; ?>
                </form>
            </aside>
            ...
            <div class="archive-filtered__content">
                <div class="venues-browser__results-head">
                    <div>
                        <h2 class="venues-browser__results-title"><?php esc_html_e( 'Wedding Venue Results', 'sdweddingdirectory' ); ?></h2>
                        <p class="venues-browser__results-summary">
                            <?php
                            echo esc_html(
                                sprintf(
                                    _n( '%s venue matched your criteria.', '%s venues matched your criteria.', $venue_query->found_posts, 'sdweddingdirectory' ),
                ...
                <?php else : ?>
                    <div class="archive-empty">
                        <h3 class="archive-empty__title"><?php esc_html_e( 'No venues matched these filters.', 'sdweddingdirectory' ); ?></h3>
                        <p><?php esc_html_e( 'Try broadening the location, price range, or amenities to see more results.', 'sdweddingdirectory' ); ?></p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
