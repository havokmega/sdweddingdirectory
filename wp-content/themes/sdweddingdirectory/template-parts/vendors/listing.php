...
$results_heading = $selected_category instanceof WP_Term
    ...
    : __( 'All Wedding Vendors', 'sdweddingdirectory' );

if ( ! empty( $filters['search'] ) ) {
    $results_heading = sprintf(
        /* translators: %s: search term. */
        __( 'Results for "%s"', 'sdweddingdirectory' ),
        $filters['search']
    );
}
...
$results_summary = sprintf(
    ...
    _n( '%s vendor found', '%s vendors found', $vendor_query->found_posts, 'sdweddingdirectory' ),
    ...
    <div class="container">
        <form class="vendors-search-form" method="get" action="<?php echo esc_url( $current_url ); ?>">
            <label class="vendors-search-form__field">
                <span class="screen-reader-text"><?php esc_html_e( 'Search vendors', 'sdweddingdirectory' ); ?></span>
                <input type="search" name="vendor_search" value="<?php echo esc_attr( $filters['search'] ); ?>" placeholder="<?php esc_attr_e( 'Search vendor name or keyword', 'sdweddingdirectory' ); ?>">
            </label>

            <?php if ( $fixed_category_id && $selected_category && ! is_wp_error( $selected_category ) ) : ?>
                <div class="vendors-search-form__chip">
                    <span class="vendors-search-form__chip-label"><?php esc_html_e( 'Category', 'sdweddingdirectory' ); ?></span>
                    <strong><?php echo esc_html( $selected_category->name ); ?></strong>
                </div>
            <?php else : ?>
                <label class="vendors-search-form__field">
                    <span class="screen-reader-text"><?php esc_html_e( 'Vendor category', 'sdweddingdirectory' ); ?></span>
                    <select name="vendor_cat">
                        <option value=""><?php esc_html_e( 'All categories', 'sdweddingdirectory' ); ?></option>
            ...
            <label class="vendors-search-form__field">
                <span class="screen-reader-text"><?php esc_html_e( 'Sort vendors', 'sdweddingdirectory' ); ?></span>
                <select name="sort">
            ...
            <button class="btn btn--cta vendors-search-form__submit" type="submit">
                <?php esc_html_e( 'Search Vendors', 'sdweddingdirectory' ); ?>
            </button>
        </form>
                ...
                <form class="vendors-filter-form" method="get" action="<?php echo esc_url( $current_url ); ?>">
                    ...
                    <?php if ( ! empty( $options['pricing'] ) ) : ?>
                        <fieldset class="vendors-filter-form__group">
                            <legend class="vendors-filter-form__label"><?php esc_html_e( 'Pricing', 'sdweddingdirectory' ); ?></legend>
                    ...
                    <?php foreach ( [
                        'vendor_services'    => [ 'label' => __( 'Services', 'sdweddingdirectory' ), 'options' => $options['services'] ],
                        'vendor_style'       => [ 'label' => __( 'Style', 'sdweddingdirectory' ), 'options' => $options['styles'] ],
                        'vendor_specialties' => [ 'label' => __( 'Specialties', 'sdweddingdirectory' ), 'options' => $options['specialties'] ],
            ...
            <div class="archive-filtered__content">
                ...
                <?php else : ?>
                    <div class="archive-empty">
                        <h3 class="archive-empty__title"><?php esc_html_e( 'No vendors matched those filters.', 'sdweddingdirectory' ); ?></h3>
                        <p><?php esc_html_e( 'Try broadening your category or clearing a few filter selections.', 'sdweddingdirectory' ); ?></p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
