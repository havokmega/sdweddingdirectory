<?php
/**
 * SDWD Core — Venue Meta Boxes
 *
 * Same as vendor (business info, social, hours, pricing)
 * PLUS location and capacity.
 */

defined( 'ABSPATH' ) || exit;

add_action( 'add_meta_boxes', 'sdwd_venue_meta_boxes' );
add_action( 'save_post_venue', 'sdwd_save_venue_meta', 10, 2 );

/**
 * Register venue meta boxes.
 */
function sdwd_venue_meta_boxes() {
    add_meta_box(
        'sdwd-venue-business',
        __( 'Business Info', 'sdwd-core' ),
        'sdwd_venue_business_cb',
        'venue',
        'normal',
        'high'
    );
    ...
    add_meta_box(
        'sdwd-venue-location',
        __( 'Location', 'sdwd-core' ),
        'sdwd_venue_location_cb',
        'venue',
        'normal',
        'high'
    );
    ...
    add_meta_box(
        'sdwd-venue-capacity',
        __( 'Capacity', 'sdwd-core' ),
        'sdwd_venue_capacity_cb',
        'venue',
        'side',
        'default'
    );
 ...
 */
function sdwd_venue_business_cb( $post ) {
    wp_nonce_field( 'sdwd_venue_meta', 'sdwd_venue_meta_nonce' );

    $fields = [
        'sdwd_company_name'    => __( 'Venue Name', 'sdwd-core' ),
...
/**
 * Location meta box (venue only).
 */
function sdwd_venue_location_cb( $post ) {
    $fields = [
        'sdwd_street_address' => __( 'Street Address', 'sdwd-core' ),
        'sdwd_city'           => __( 'City', 'sdwd-core' ),
        'sdwd_zip_code'       => __( 'Zip Code', 'sdwd-core' ),
    ];
    ?>
    <table class="form-table sdwd-meta-table">
        <?php foreach ( $fields as $key => $label ) : ?>
...
/**
 * Capacity meta box (sidebar).
 */
function sdwd_venue_capacity_cb( $post ) {
    $capacity = get_post_meta( $post->ID, 'sdwd_capacity', true );
    ?>
    <label for="sdwd_capacity"><?php esc_html_e( 'Max Guest Count', 'sdwd-core' ); ?></label>
    <input type="number" id="sdwd_capacity" name="sdwd_capacity" value="<?php echo esc_attr( $capacity ); ?>" min="0" style="width:100%; margin-top:6px;">
    <?php
}

/**
 * Social Media meta box (repeatable rows) — same pattern as vendor.
 */
function sdwd_venue_social_cb( $post ) {
...
/**
 * Business Hours meta box — same pattern as vendor.
 */
function sdwd_venue_hours_cb( $post ) {
...
/**
 * Pricing Tiers meta box — same pattern as vendor.
 */
function sdwd_venue_pricing_cb( $post ) {
 ...
 * Save venue meta.
 */
function sdwd_save_venue_meta( $post_id, $post ) {
    if ( ! isset( $_POST['sdwd_venue_meta_nonce'] ) || ! wp_verify_nonce( $_POST['sdwd_venue_meta_nonce'], 'sdwd_venue_meta' ) ) {
        return;
    }
    ...
    // Text fields.
    $text_fields = [
        'sdwd_company_name', 'sdwd_email', 'sdwd_phone', 'sdwd_company_website',
        'sdwd_street_address', 'sdwd_city', 'sdwd_zip_code',
    ];
