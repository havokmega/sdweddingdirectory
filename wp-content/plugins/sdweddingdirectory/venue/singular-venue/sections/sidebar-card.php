<?php
/**
 * Venue Singular - Section 3: Sticky Sidebar Card
 */
...
$location_terms = wp_get_post_terms( $post_id, sanitize_key( 'venue-location' ) );
$location_label = esc_html__( 'San Diego', 'sdweddingdirectory' );

if( ! is_wp_error( $location_terms ) && ! empty( $location_terms ) ){
    $location_label = $location_terms[0]->name;
}
...
if( $starting_price === '' ){
    $fallback_price = get_post_meta( $post_id, sanitize_key( 'venue_price' ), true );
    $fallback_price = $fallback_price !== '' ? preg_replace( '/[^0-9.]/', '', sanitize_text_field( $fallback_price ) ) : '';
    $starting_price = $fallback_price !== '' ? number_format_i18n( (float) $fallback_price, 0 ) : esc_html__( 'Custom', 'sdweddingdirectory' );
}

$booked_dates = get_post_meta( $post_id, sanitize_key( 'vendor_booked_dates' ), true );
if( ! is_array( $booked_dates ) ){
    $booked_dates = get_post_meta( $post_id, sanitize_key( 'venue_booked_dates' ), true );
}
...
if( ! is_user_logged_in() ){
    ...
    $request_attrs = sprintf( 'data-venue-id="%1$s" data-event-call="0" id="%2$s"', absint( $post_id ), esc_attr( wp_unique_id( 'sdwd_venue_request_' ) ) );
}
?>
<div class="sd-profile-sidebar-sticky" id="<?php echo esc_attr( $sidebar_element_id ); ?>">
    <div class="card shadow">
        <div class="card-body">
            <h1 class="h4 fw-bold mb-1"><?php echo esc_html( $business_name ); ?></h1>
            ...
            <?php } ?>
            <p class="mb-2 sd-profile-meta"><i class="fa fa-map-marker"></i> <?php echo esc_html( $location_label ); ?></p>
            ...
            <?php
            $sidebar_capacity = get_post_meta( $post_id, sanitize_key( 'venue_seat_capacity' ), true );
            if( absint( $sidebar_capacity ) > 0 ){
            ?>
            <p class="mb-2 sd-profile-meta"><i class="fa fa-users"></i> <strong><?php echo absint( $sidebar_capacity ); ?></strong> <?php esc_html_e( 'guests', 'sdweddingdirectory' ); ?></p>
            <?php } ?>
